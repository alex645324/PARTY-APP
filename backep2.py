...
def webhook():
    ...
    # Handle selected party details
        ...
        match = re.match(r'^party(\d+)$', incoming_msg)
        if match:
            party_number = int(match.group(1))
            parties = list(db.collection('parties').stream())
            if 1 <= party_number <= len(parties):
                selected_party = parties[party_number - 1]
                party_id = selected_party.id
                update_user(from_number, {'selected_party': party_id})
                party_details = get_party_details(party_id)

                if user_data.get('finding_party', False):
                    msg.body(
                ...
                elif user_data.get('partygoer_mode', False):
                    msg.body(
        ...
        # handling error messages 
            ...
            if user_data.get('party_list_shown', False):
        # User is in the party finding or partygoer section
                msg.body("Invalid input. Please type 'party' followed by the number of the party (e.g., 'party2').")
            else:
        # User is not in any specific section
                msg.body("Invalid command. Type 'menu' to see available commands.")
...
# # Function to handle Twilio webhook requests
# def main(request):
#     incoming_msg = normalize(request.values.get('Body', '').strip()).lower()
#     from_number = request.values.get('From', '')
...
#     # Define the menu message
#     menu_message = (
#         "Hey, what's up! 😎 I’m your go-to guide for all things party. "
#         "Here’s how you can navigate around:\n\n"
#         "To find what parties are lit tonight, type: 🕺\n"
#         "To review parties and get free drinks (approval process), type: 🎉\n"
#         "To learn more about us, check this out: [insert website link]"
#     )
...
#             party_data = party.to_dict()
#             party_id = party.id
#             if party_data.get("has_reviews", False):  # Only add the party if it has reviews
#                 party_prompt = f"party{i}"
#                 party_list.append(
#                     f"   {party_data.get('type', 'Party')}\n"
#                     f"   📍 {party_data.get('address', 'Unknown Address')}\n"
...
#         if partygoer_approved:
#             msg.body("You're already approved as a partygoer!")
#         else:
#             msg.body("Your request to be a partygoer has been received. Please wait for approval.")

#     # Handle check-in (user sends 'boy' or 'girl')
#     elif incoming_msg.startswith(('boy', 'girl')):
#         if not user_data.get('partygoer_approved', False):
#             msg.body("You are not approved as a partygoer. Please wait for admin approval.")
#         else:
